...
if(!$anexo) $anexo = $codAnexo;


if(!$radicado) {$radicado = $nurad;}
...
if(!empty($arrAnexos)){
  ...
  foreach($arrAnexos->desc_anexos as $ANEX_CODIGO => $desc_anexos) {
    if($desc_anexos) $desc_anex[$ANEX_CODIGO] = $desc_anexos; 
  }
...
if($sendMail=="Enviar Correo" and $env_email!=1 ){
   ...
   /* $tipo_E="select anex_tipo_envio from anexos       echo "Entro...xx ";where radi_nume_salida= $nurad";
    $tipo_env   = $db->conn->Execute($tipo_E);
    $tipo_envio= $tipo_env->fields["ANEX_TIPO_ENVIO"];*/
