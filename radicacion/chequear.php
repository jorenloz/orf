...
if($primera!=1 and $Submit=="Buscar" and
 ...
 $estoybuscando = 1;
	...
	if (!$rsCheck and $tpBuscarSel=="ok"){
		...
		if($tpBuscarSel=="ok"){ ?>
      <label class="input">
        <input name='radicadopadre' type='radio' value='' title='Radicado No <?=$nume_radi ?>' class="input"> NO TIENE PADRE
      </label>
		<? }
		...
		while(!$rsCheck->EOF){
            ...
            <td><?=$fecha ?></td>
					...
					<tr>
            <td><b><?=ucfirst(strtolower($nombreTp1))?></td>
            <td>
              <?
            ...
            <td><?=$cuentai?></td>
					<tr>
            <td><b><?=ucfirst(strtolower($nombreTp2))?></b></td>
            <td><?=$nombret_us2 ?></td>
            <td><b>Doc Asociado</b> </td>
            <td><?=$asociado?></td>
					</tr>
					<tr>
            <td><b><?=ucfirst(strtolower($nombreTp3))?></b></td>
            <td><?=$nombret_us3." - ".$dignatario3 ?>
            <?=$dato ?></td>
            <td><b>Asunto</b></td>
            <td><?=$asunto ?></td>
					</tr>
            ...
            <td><?=$tip_doc_des?></td>
            <td><b>Usuario actual Responsable</b></td>
            <td><?=$usua_actu?></td>
					</tr>
					<tr>
            <td><b>Dependencia Actual</b></td>
            <td><?=$depe_actu?></td>
            <td><b>Dependenica Radica</b></td>
            <td><?=$depe_radi?></td>
					</tr>
          </table>
          <br/>
            ...
            <table class="table table-bordered table-striped" width="100%">
              <tr>
                <td class="titulosError">
                  <center><font color="red">¡No se encontraron resultados!.</font> </center>
                </td>
              </tr>
            </table>
          </center>';
