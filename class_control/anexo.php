	...
	while (!$rs->EOF)
 		{
		   ...
		   $path_anexos[$codi_anexos[$i]] = "/bodega/".substr($codi_anexos[$i], 0, 4)."/".substr($codi_anexos[$i],4,3)."/docs/".$rs->fields["ANEX_NOMB_ARCHIVO"];
		   $adjuntos[$codi_anexos[$i]]    = $rs->fields["ANEX_ADJUNTOS_RR"];
		   
		     
		   
		   //echo "<hr>***** Numero $i --> *** ".$this->codi_anexos[$i];
		   //echo $rs->fields['ANEX_CODIGO']."**** ";
		  // echo $desc_anexos."gggggg ";
		   
		   //var_dump ($codi_anexos);
		   //
		   //echo $rs->fields['ANEX_DESC']."**** ";
		   //echo $rs->fields['ANEX_NOMB_ARCHIVO']."**** ";
