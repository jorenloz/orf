...
if (isset($_REQUEST['clave']) && isset($_SESSION["usua_perm_firma"])){
    $archPdfC= str_replace("./.",$_SESSION['RUTA_ABSOLUTA'],$linkarchivo);
...
if ($_SESSION['apiFirmaDigital']!='certicamara'){
    ...
    $commandFirmado='java  -jar '.$_SESSION['RUTA_ABSOLUTA'].'/include/JSignPdf-1.5.1/JSignPdf.jar '.str_replace('odt','pdf',$nombreArchivo).' -kst PKCS12  -ksf '.$_SESSION['RUTA_ABSOLUTA'].'/bodega/firmas/'.$usua_doc.'.p12   -ksp '.$clave.' --font-size 7    -r \'Firmado al Radicar en OrfeoGPL\'  -V -v   --img-path '.$_SESSION['RUTA_ABSOLUTA'].'/imagenes/gnu.gif --render-mode  GRAPHIC_AND_DESCRIPTION -llx 0 -lly 0 -urx 550 -ury 27';
...
$fileToSignPath = str_replace(".odt",".pdf",$_SESSION['RUTA_ABSOLUTA']."/bodega/tmp/workDir/$nombreArchivo");
//ruta para guardar el archivo firmado
$fileSignedPath = $_SESSION['RUTA_ABSOLUTA']."/bodega/$pathFinal";
...
if ($_SESSION['apiFirmaDigital']!='certicamara'){
    $commandFirmado='java  -jar '.$_SESSION['RUTA_ABSOLUTA'].'/include/JSignPdf-1.5.1/JSignPdf.jar '.str_replace('docx','pdf',$nombreArchivo).' -kst PKCS12  -ksf '.$_SESSION['RUTA_ABSOLUTA'].'/bodega/firmas/'.$usua_doc.'.p12   -ksp '.$clave.' --font-size 7    -r \'Firmado al Radicar en OrfeoGPL\'  -V -v   --img-path '.$_SESSION['RUTA_ABSOLUTA'].'/imagenes/gnu.gif --render-mode  GRAPHIC_AND_DESCRIPTION -llx 0 -lly 0 -urx 550 -ury 27';
    if (exec($commandFirmado)=="INFO  Finished: Creating of signature failed."){
			unset($answer);
			$answer=array();
                    saveMessage('error',"Clave de firma digital erronea");
    ...
    rename(str_replace(".docx","_signed.pdf",$nombreArchivo), "../../$pathFinal" );
}else  {
include ("include/apiCerticamara/PdfSign.php");
//ruta archivo a firmar
$fileToSignPath = str_replace(".docx",".pdf",$_SESSION['RUTA_ABSOLUTA']."/bodega/tmp/workDir/$nombreArchivo");
//ruta para guardar el archivo firmado
$fileSignedPath = $_SESSION['RUTA_ABSOLUTA']."/bodega/$pathFinal";
//ruta del certificado de firma
//$signP12Path = "$root/resources/certificate/cra.p12";
