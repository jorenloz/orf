...
<form method="GET" action="cambiarFechaVencimiento.php" >
 ...
 <input id="newFechaVencimiento" name="newFechaVencimiento" type="text" data-provide="datepicker" data-date-format="YYYY/MM/DD" data-date-end-date="0d" value="<?=$radi_fech_vcmto;?>">
 <br>
 <input type=submit value="Cambiar Fecha de Vencimiento" name="cambiarFecha" class="btn btn-default btn-xs"    >
 <input name="Cerrar" type="button" class="btn btn-default btn-xs" id="envia22" onClick="opener.regresar(); window.close(); " value="Cerrar">	
...
if($newFechaVencimiento && $numRad && $cambiarFecha){
  $iSql = "UPDATE radicado set fech_vcmto='$newFechaVencimiento' where radi_nume_radi=$numRad";
  $db->conn->query($iSql);
  ?>
  <script>
  opener.regresar(); window.close();
  </script>
  <?php
}
?>
