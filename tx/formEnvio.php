...
//Archivo Requiere Expediente20169000013131 
...
$regsTx =  implode(",", array_keys($checkValue));
	  ...
	  $radicadosTx .= empty($radicadosTx)? $record_id : ",$record_id";
		...
		{	case  7:
			...
			if($codTx == 12){
				...
				$isqlw="select u.usua_login as RADI_USU_ANTE, h.sgd_ttr_codigo, h.usua_doc, u.usua_esta from hist_eventos h, usuario u where h.radi_nume_radi =  ".$record_id." and h.sgd_ttr_codigo in (9,12,11,16,13) and h.usua_doc=u.usua_doc order by hist_fech desc limit 1";
				//$db->conn->debug = true;
          ...
          $rad = new Radicacion($db);
					...
					if ((($codTx == 9 or $codTx == 12)  and ($codusuario != 1 and $codusuario != $usuarioEnrutador)) or $codTx == 16 or $codTx == 13)
					{
...
{	case 7:
	...
	case 9:	$whereDep = "and u.depe_codi=$depsel ";
		...
		$sql = "SELECT DEPENDENCIA_OBSERVA, DEPENDENCIA_VISIBLE FROM DEPENDENCIA_VISIBILIDAD WHERE DEPENDENCIA_OBSERVA=$dependencia and DEPENDENCIA_VISIBLE = " . $depsel;
			...
			{	//Se adicionan las dependencias que puedan ver a otras en la consulta
				...
				$usuario_publico = substr($usuario_publico , 0, strlen($usuario_publico) - 1). ") AND USUA_ESTA='1') AND u.id in (select distinct autu_id from autp_permisos ap, autr_restric_grupo ar,autm_membresias am
 where am.autg_id=ar.autg_id and ar.autp_id=ap.id and upper(ap.nombre)='USUARIO_PUBLICO') ";
			...
			{	
				$iSqlEnrutadores = "select u.usua_codi, u.usua_login, m.autu_id,r.autg_id,r.autp_id, u.depe_codi from autr_restric_grupo r join autm_membresias m on (r.autg_id=m.autg_id)
				...
				$whereReasignar = " and (u.usua_codi=1 or $enrutadores=u.usua_codi) ";
				$usDefault = 1;
			...
			{	
				...
				while(!$rs->EOF)
				{
					...
					if($usuCodi==1  || $usuCodi==$enrutadores) 
						{
						$defaultUs = "selected"; 
  ...
  <?php
		if ($codTx=="9" || $codTx=="8")
		{
			$scriptCargarUsuarios = " onClick=".'"'."getUsuarios('usuariosInformar',document.getElementById('coddepeinf').value,0)".'";';
    ...
    onclick="informarUsuario('<?=$radicadosTx?>', document.getElementById('coddepeinf').value,document.getElementById('usuariosInformar').value,<?=$observaInf?>);" 
...
<script>
 ...
 function listaInformados(arrRadicados){
  $.post( "../tx/ajax_informarLista.php", { arrRadicados: arrRadicados, idObjetoHtml: "usuariosInformados" })
    .done(function( data ) {
        document.getElementById('usuariosInformados').innerHTML=data;
    });                          
 }
