...
function crearExpediente($nurad,$usuario,$anoExp,$fechaExp,$codiSRD,$codiSBRD,$codiProc,$digCheck,$tmr){
	...
	$sql= "select USUA_CODI,DEPE_CODI,USUA_DOC from usuario where upper(usua_email) = upper ('".$usuario."')";
	$rs = $db->conn->query($sql);
	while (!$rs->EOF){
...
function getRadsExpediente($numeroExpediente="", $searchText=""){
 // return "ws entro ssss";
  ...
  if($searchText!='')$sq=" and upper(sexp.sgd_sexp_parexp1) = upper ('".$searchText."')";
  if($numeroExpediente!='')$sq2=" and exp.sgd_exp_numero='".$numeroExpediente."'";
  //Aqui busco la informacion necesaria del usuario para la creacion de expedientes
  //$db->conn->debug = true;
  $sql= "select exp.RADI_NUME_RADI, r.ra_asun, r.RADI_FECH_RADI, sexp.sgd_sexp_parexp1
           ...
           where  sexp.sgd_exp_numero=exp.sgd_exp_numero and r.radi_nume_radi=exp.radi_nume_radi".$sq.$sq2;
  //return $sql;
  $rs = $db->conn->query($sql);
  $exp='';
  while(!$rs->EOF){
       ...
       $etiqueta1 = $rs->fields['SGD_SEXP_PAREXP1'];
       $exp.=$radi_nume_radi.'||'.$asunto.'||'.$fecha.'||'.$etiqueta1.'<<<';
       $rs->MoveNext();
  }
   $exp=substr($exp,0,strlen($exp)-3);
...
function crearAnexo($radiNume,$file,$filename,$correo,$descripcion){
	...
	if(!$error){
		...
		$consulta="INSERT INTO ANEXOS (ANEX_CODIGO,ANEX_RADI_NUME,ANEX_TIPO,ANEX_TAMANO,ANEX_SOLO_LECT,ANEX_CREADOR,
		            ANEX_DESC,ANEX_NUMERO,ANEX_NOMB_ARCHIVO,ANEX_ESTADO,SGD_REM_DESTINO,ANEX_FECH_ANEX, ANEX_BORRADO,SGD_DIR_TIPO,ANEX_DEPE_CREADOR,SGD_TPR_CODIGO) 
		            VALUES('$nombreAnexo',$radiNume,$tipoAnexo,$tamanoAnexo,'n','".$us[5]."','$descripcion'
		            ,$numAnexo,'$nombreAnexo.$extension',0,1,$fechaAnexado, 'N',1,'".$us[2]."',0)";
		...
		$cod = $rs->fields['ANEX_CODIGO'];
                if($cod){
                    $insertaHistorico = "insert into HIST_EVENTOS(RADI_NUME_RADI,DEPE_CODI,USUA_CODI,USUA_CODI_DEST,
							 ...
							 values ( $radiNume , '" . $us[2] . "', " . $us[1] . 
							 " , " . $us[1] ." , '" . $us[2] . "' , '" . $us[3] . "' , '" . $us[3] . "' ,0 ,
							 'Anexo WebService',(SYSDATE+0))"; 
                    $rs=$db->query($insertaHistorico);
	}
	}
