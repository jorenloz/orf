...
if ($radicados){
  $radicados = str_replace( "''","'",$rads);
    ...
    $whereFiltro = "and dir.radi_nume_radi||'_'||dir.sgd_dir_tipo  in($radicados)";
...
$isql = "SELECT a.SGD_DIR_TIPO, ".	$RADI_NUME_SALIDA." as RADI_NUME_SALIDA, ".$radi_nume_deri." AS RADI_NUME_DERI, b.RA_ASUN
		...
		FROM ANEXOS a,RADICADO b, sgd_dir_drecciones dir  WHERE a.radi_nume_salida=b.radi_nume_radi ".$whereFiltro .
		" AND anex_estado=3 and dir.radi_nume_radi=a.radi_nume_salida   
		".$comb_salida .
			...
			{	
				...
				while (!$rsEnviar->EOF)
				{
					...
					if (!$codigo_envio){	
						...
						$rsUpdate = $db->query($isql);
						echo "<hr>";
						$isql = "update SGD_DIR_DRECCIONES set SGD_dir_enviado=1 where RADI_NUME_RADI =$verrad_sal and sgd_dir_tipo=$rem_destino";
            $rsUpdate = $db->query($isql);
