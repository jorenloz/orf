...
require_once($ruta_raiz.'/include/CRA_phpmailer/class.phpmailer.php'); require_once($ruta_raiz."/conf/configPHPMailer.php"); //$depeRecibeFormularioWeb;
include_once "../config.php";

/*EXPEDIENTES*/
require_once "$ruta_raiz/include/tx/Historico.php";
require_once("$ruta_raiz/class_control/TipoDocumental.php");
...
//Aqui se cambia de igual forma la variable $adjuntosSubidos. $numeroExpediente
require_once "validarDatosPost.php";
...
if($errorFormulario==0){
	if($anonimo==1){
		die("<br> No se puede enviar una peticion ANONIMA");
		//Esto es anónimo
	...
	*/
	...
	if(!isset($_GET['sigla'])){$_SESSION['sigla'] = "0";} else{$_SESSION['sigla']=$_GET['sigla'];}
	$_SESSION['usuario']=1;
	if(!$_SESSION['dependencia']) $_SESSION['dependencia']=900;
	$dependenciaRad = $_SESSION['dependencia'];

	if(!isset($_GET['radicado'])){$_SESSION['radicado']=NULL;}else{$_SESSION['radicado']=$_GET['radicado'];}

	if(!isset($_GET['documento_destino'])){$_SESSION['documento_destino']="";}else{$_SESSION['documento_destino']=$_GET['documento_destino'];}	
	$secRadicaFormularioWeb="secr_tp2_".$secRadicaFormularioWeb;
    ...
    if($tipoDocumento > 0 && $tipoDocumento != 5 ){
		...
		$ins_ciu="insert into sgd_ciu_ciudadano values($tipdoc,".$num_ciu.",'".mb_strtoupper($_SESSION['nombre_remitente'],"utf-8")."','".mb_strtoupper($_SESSION['direccion_remitente'],"utf-8")."','".mb_strtoupper($_SESSION['apellidos_remitente'],"utf-8")."','','".$_SESSION['telefono_remitente']."','".$_SESSION['email']."',".$_SESSION['muni'].",".$_SESSION['depto'].",'".$_SESSION['cedula']."','1','170')";

		$rs_ins_ciu=$db->conn->Execute($ins_ciu);
		//inserta en sgd_dir_direcciones
		$ins_dir="insert into sgd_dir_drecciones(sgd_dir_codigo,sgd_dir_tipo,sgd_oem_codigo,sgd_ciu_codigo,radi_nume_radi,sgd_esp_codi,muni_codi,dpto_codi,sgd_dir_direccion,sgd_dir_telefono,sgd_sec_codigo,sgd_dir_nombre,sgd_dir_apellido,sgd_dir_nomremdes,sgd_trd_codigo,sgd_dir_doc,sgd_dir_mail)
...
values ($numeroRadicado, CURRENT_TIMESTAMP,'$tipoSolicitud',".$_SESSION['mrec_codi'].",".$_POST['entidad'].",
...
,'". $descripcionAnexos ."', ";
	...
	if($rs_ins_rad=$db->conn->Execute($ins_rad)){
		...
		/*Insertamos el tipo de documento*/
            ...
            $record["RADI_NUME_RADI"] = $numeroRadicado;
            $record["DEPE_CODI"]      = $_SESSION['dependencia'];
            $record["USUA_CODI"]      = '54';
            $record["USUA_DOC"]       = '101010101010';
            $record["SGD_MRD_CODIGO"] = '607';
            $record["SGD_RDF_FECH"]   = $db->conn->OffsetDate(0,$db->conn->sysTimeStamp);
		$db->insert("SGD_RDF_RETDOCF", $record, "true");

	}else{
		die("ERROR - FAVOR COMUNICARSE CON EL ADMINISTRADOR");
	}
...
values($dependenciaRad,CURRENT_TIMESTAMP,$usuaRadicaFormularioWeb,$numeroRadicado,'$obsHistorico',".$_SESSION['usuario'].",'22222222',2,'".$_SESSION['documento_destino']."',".$_SESSION['dependencia'].")";
    ...
    $uploader->moverArchivoCarpetaBodega($numrad);

    
	//trae usualogin
	$sql_login="select usua_login from usuario where usua_codi=$usuaRadicaFormularioWeb and depe_codi=$depeRadicaFormularioWeb";
...
//			$tipoCodigo   = 24; .  
			//UPDATE ANEXOS SET anex_creador = 'USERWEB' 
			if(!$rs_tipoAnexo->EOF){
				$ins_anex="INSERT 
						INTO ANEXOS(
                         ANEX_RADI_NUME
						...
						,ANEX_FECH_ANEX
						,SGD_APLI_CODI)
					values(".$numeroRadicado."
						,".$numeroRadicado.sprintf("%05d",($i+1))."
						,".$tipoCodigo."
						,".$uploader->sizes[$i]."
						,'S'
						,'USERWEB'
						,'".$uploader->nombreSubidos[$i]."'
						,1
...
#insert into sgd_exp_expediente (sgd_exp_numero,radi_nume_radi,depe_codi,usua_codi,usua_doc) values ($expediente, $radicado,110,54,101010101010 );
$codusuario = 54; //Codigo de usuario web
$usua_doc = 101010101010; //Cedula del usuario web

if ($numeroExpediente != "") {

		#echo "Radicado :".$numeroRadicado."<br>";
		#echo "numeroExpediente :".$numeroExpediente."<br>";
		$ins_exp = "insert into sgd_exp_expediente (sgd_exp_numero,radi_nume_radi,depe_codi,usua_codi,usua_doc,sgd_exp_fech) values ('".$numeroExpediente."', '".$numeroRadicado."',".intval($_SESSION["depeRadicaFormularioWeb"]).",".intval($codusuario).",".intval($usua_doc).",'".date('Y-m-d H:i:s')."' )";
		$rs_ins_expediente = $db->conn->Execute($ins_exp);
	    ...
	    $dependenciaExp = $_SESSION["depeRadicaFormularioWeb"];
	    $anoExp = date('Y');
	    ...
	    if ($numeroExpediente != "") {
	    	//Cejebuto
	    	$arrParametro[1] = $_SESSION['nombre_remitente']." ".$_SESSION['apellidos_remitente'];
	    	$arrParametro[2] = $_POST['numid'];
	    	$arrParametro[3] = "";
	    	$arrParametro[4] = "";
	    	$arrParametro[5] = "";

			$expediente->crearExpedienteExterno($numeroExpediente, $numeroRadicado, $dependenciaExp, $codusuario, $usua_doc, $usua_doc, $codiSRD, $codiSBRD, 'false', date('Y-m-d H:i:s'), 0, $arrParametro);

		$ins_exp = "insert into sgd_exp_expediente (sgd_exp_numero,radi_nume_radi,depe_codi,usua_codi,usua_doc,sgd_exp_fech) values ('".$numeroExpediente."', '".$numeroRadicado."',".intval($_SESSION["depeRadicaFormularioWeb"]).",".intval($codusuario).",".intval($usua_doc).",'".date('Y-m-d H:i:s')."' )";
			$rs_ins_expediente = $db->conn->Execute($ins_exp);
	    }else{
	    	echo "No se pudo crear expediente"; 
	    }
...
$pdf->Image('../img/IK.banerPDF.jpg',20,20,75);
$pdf->SetFont('Arial','',7);
...
$pdf->SetFont('','B');
$pdf->Text(12,85,textoPDF(html_entity_decode($entidad_largo,ENT_QUOTES,"UTF-8"))); $pdf->SetFont('','');
...
$db->conn->Execute($sqlu);
    ...
    $usMailSelect  = "orfeo@infometrika.com"; 
    ...
    $mail->Subject    = "Radicado " .$numeroRadicado . "IK";
    ...
    $asu .= "<hr>Sistema de gestion Orfeo. http://www.infometrika.com";
...
<table width="80%" border="0" align="center" cellpadding="0"
	...
	<tr>
		<td align="center"><br />
		<img src="../img/IK.logoEntidad.png" width="45px" /></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	</tr>
	...
	*/ ?>
	...
	<td align="center">
		Su solicitud ha sido registrada de forma exitosa, y se empezará a validar la información.
	</td>
	...
	*/?>
	...
	<tr>
		<td align="center">
			...
			<input type="button" name="Cerrar"	value="Cerrar"	onclick="window.location='http://www.infometrika.com/wordpress/'" />
		</td>
	</tr>
	<tr>
		<td align="center">&nbsp;</td>
